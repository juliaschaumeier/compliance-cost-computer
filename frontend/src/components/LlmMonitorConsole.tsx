  ...
  return createPortal(
    <div className="fixed inset-0 z-[90] flex items-end justify-end bg-slate-900/30 p-3 sm:p-4">
      <section className="flex h-[80vh] w-full max-w-[1150px] flex-col overflow-hidden rounded-2xl border border-slate-300 bg-white shadow-2xl">
          ...
          <div className="flex items-center gap-2">
            <span
              className={`rounded-full px-2 py-1 text-[10px] font-semibold ${
                streamError
                  ? "ccc-status-warning bg-amber-100 text-amber-800"
                  : "ccc-status-success bg-emerald-100 text-emerald-800"
              }`}
            >
        ...
        {streamError && (
          <div className="ccc-status-warning border-b border-amber-200 bg-amber-50 px-4 py-2 text-xs font-semibold text-amber-800">
            {streamError}
          </div>
        )}
                        ...
                        <span
                          className={`rounded px-1.5 py-0.5 text-[10px] font-semibold ${
                            row.answer_state === "invalid"
                              ? "ccc-status-error bg-rose-100 text-rose-800"
