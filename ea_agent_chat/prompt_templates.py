...
class PromptTemplate:
    def __init__(self, name, text, json_template_name=None, regelung_kurzform=None):
        self.name = name
        self.text = text
...
daran und schließen Sie keine weiteren Informationen oder Decorators ein. Diagramme bilden Sie in der Markdown Mermaid 
Syntax ab, die derzeit gültige Syntax können Sie unter dieser URL einsehen: 
...
"""
        super().__init__(name='instructions', text=text, json_template_name=None)
...
""".format(regelung_kurzform, regelung_text, json_templates.json_ea_yearly)
        super().__init__(name='jaehrlicher_erfuellungsaufwand', text=text, json_template_name='json_ea_yearly',
                         regelung_kurzform=regelung_kurzform)


class ChooseBestAnswerFromList(PromptTemplate):
    def __init__(self, answered_task, list_of_answers):
        tagged_answers = ""
...
<Aufgabe_Start>{1}<Aufgabe_Ende>\n\n{2}
     """.format(len(list_of_answers), answered_task, tagged_answers)
        super().__init__(name='choose_best_answer_from_list', text=text, json_template_name=None)
...
""".format(json_templates.sources_template, json_templates.sources_verification_template, list_of_sources)
        super().__init__(name='verify_web_sources', text=text, json_template_name='sources_verification_template')


class TestPrompt(PromptTemplate):
    def __init__(self):
        text = """ Bitte beantworten Sie die folgende Frage: Was ist 2 + 2 + eine kleine zufällige Zahl? """
        super().__init__(name='test_prompt', text=text, json_template_name=None)
...
class FollowUpTestPrompt(PromptTemplate):
    def __init__(self):
        ...
        super().__init__(name='follow_up_test_prompt', text=text, json_template_name=None)
