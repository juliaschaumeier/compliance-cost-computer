...
def test_case_groups_bulk_update_propagates_to_mirror_targets(test_client):
    ...
    Ziel nach manuellen Edits auseinander und die Gesamtkosten fuer den
    Spiegel-Normadressaten werden falsch."""
    session_id, _ = db.upsert_session("MIRROR-PROPAGATE", "test-model")
    ...
    db.replace_mirror_matches(
        ...
        [
            {
                "mirror_anchor_key": "shared",
                "shared_situation": "Antrag mit Bearbeitung",
    ...
    response = test_client.post(
        "/case-groups/bulk-update",
        json={
            "app_session_id": "MIRROR-PROPAGATE",
            "rows": [
                {
                    "case_group_id": admin_case_group_id,
                    "addressees_current": 25,
                    "annual_frequency_current": 2,
                    "addressees_proposed": 30,
                    "annual_frequency_proposed": 2,
                }
            ],
        },
    ...
    business_rows = db.list_case_groups_for_session_and_addressee(session_id, BUSINESS)
    business_row = next(
        row for row in business_rows if int(row["case_group_id"]) == business_case_group_id
    )
    # Nach Propagation muessen die effektiven Werte des Ziels die neuen
    # Source-Werte widerspiegeln.
    assert business_row.get("addressees_current_edited") == 25
    assert business_row.get("annual_frequency_current_edited") == 2
    assert business_row.get("addressees_proposed_edited") == 30
    assert business_row.get("annual_frequency_proposed_edited") == 2


def test_case_groups_bulk_update_does_not_overwrite_directly_edited_mirror_target(
    test_client,
):
    """Wenn der Nutzer beide Seiten eines Spiegelpaares gleichzeitig
    editiert, darf die automatische Propagation die direkte Eingabe der
    Zielseite nicht stillschweigend ueberschreiben."""
    session_id, _ = db.upsert_session("MIRROR-BOTH-EDITED", "test-model")
    admin_process_id = db.insert_process(
        session_id, "Admin", "Beschreibung", norm_addressee=ADMINISTRATION
    )
    admin_cg_id = db.insert_case_group(
        session_id,
        admin_process_id,
        "Admin CG",
        "Beschreibung",
    ...
    business_process_id = db.insert_process(
        session_id, "Business", "Beschreibung", norm_addressee=BUSINESS
    )
    business_cg_id = db.insert_case_group(
        session_id,
        business_process_id,
        "Business CG",
        "Beschreibung",
        ...
        [
            {
                ...
                "source_process_id": admin_process_id,
                "target_process_id": business_process_id,
                "source_case_group_id": admin_cg_id,
                "target_case_group_id": business_cg_id,
                "relation_type": "one_to_one",
                "sync_addressees": True,
                "sync_frequency": True,
                "sync_cases": True,
            ...
            },
            {
                ...
                "source_process_id": business_process_id,
                "target_process_id": admin_process_id,
                "source_case_group_id": business_cg_id,
                "target_case_group_id": admin_cg_id,
                "relation_type": "one_to_one",
                "sync_addressees": True,
                "sync_frequency": True,
                "sync_cases": True,
    ...
    response = test_client.post(
        "/case-groups/bulk-update",
        json={
            "app_session_id": "MIRROR-BOTH-EDITED",
            "rows": [
                {
                    "case_group_id": admin_cg_id,
                    "addressees_current": 25,
    ...
    admin_row = next(
        row
        for row in db.list_case_groups_for_session_and_addressee(session_id, ADMINISTRATION)
        if int(row["case_group_id"]) == admin_cg_id
    )
    business_row = next(
        row
        for row in db.list_case_groups_for_session_and_addressee(session_id, BUSINESS)
        if int(row["case_group_id"]) == business_cg_id
    )
    # Beide direkten Eingaben muessen erhalten bleiben.
    assert admin_row.get("addressees_current_edited") == 25
    assert business_row.get("addressees_current_edited") == 40
