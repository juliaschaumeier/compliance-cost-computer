...
def test_projection_fallback_when_no_overlap():
    # Defensive edge-case: regulation does not belong to the run's NA.
    # Upstream should have filtered it; builder catches it anyway so an empty
    # array is never silently sent to the LLM.
    reg = _multi_na_regulation()
    reg["applies_to_business"] = 0  # Admin only
    payload = build_vorgaben_payload([reg], norm_addressee=CITIZENS)
    # No overlap -> falls back to original list
    assert payload[0]["normadressaten"] == [ADMINISTRATION]
